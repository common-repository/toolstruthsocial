<?php namespace ToolsTruthSocialWPSettings;
...
class MultiField extends Field {
	...
	public function renderSingleFieldGroup() {
		...
		if($this->limit != 1) {
			echo '<ul id="' . esc_attr($this->id) . '" class="multi-field-wrapper" data-limit="' . $this->limit . '">';
	...
	public function renderMultipleFieldGroups($list_length) {
		...
		echo '<ul id="' . wp_kses($this->id, $my_allowed) . '" class="multi-field-wrapper" data-limit="' . $this->limit . '">';
